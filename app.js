...
import { dictionary, appSettings, runtime, AI_PROMPT_TEXT, startFirebaseSync, persist } from './state.js';
...
import * as syncToolManager from './sync_tool_manager.js';
import { injectModals } from './modal_elements.js';

// Inject modal structures first
injectModals();
...
function setupEventListeners() {
    ...
    els.translateWordBtn.addEventListener('click', editorManager.translateCurrentWord);

    // Close modals when clicking on the backdrop
    [els.settingsModal, els.wordModal, els.wordListModal, els.translationModal, els.syncToolModal].forEach(modal => {
        if (!modal) return;
        modal.addEventListener('click', (e) => {
            // If the click is exactly on the modal backdrop (not its children)
            if (e.target === modal) {
                if (modal === els.syncToolModal) {
                    syncToolManager.closeSyncTool();
                } else {
